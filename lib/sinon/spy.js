...
const { forEach, pop, push, slice } = commons.prototypes.array;
...
function matches(fake, args, strict) {
    ...
    if (
        ...
        samsam.deepEqual(slice(args, 0, margs.length), margs)
    ) {
...
delegateToCalls(spyApi, "yieldTo", false, "yieldTo", true, function (property) {
    throw new Error(
        `${this.toString()} cannot yield to '${commons.valueToString(
            property,
...
function createSpy(func) {
    ...
    if (typeof funk !== "function") {
        ...
        name = commons.functionName(funk);
    }
