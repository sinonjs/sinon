...
spyApi.formatters = {
    ...
    D: function (spyInstance, args) {
        if (!spyInstance.getCall || !spyInstance.getCall(0)) {
            return "";
        }

        var calledArgs = spyInstance.getCall(0).args;
        var message = "";

        for (var i = 0; i < calledArgs.length || i < args.length; i++) {
            message += "\n";
            var calledArg = i < calledArgs.length ? sinonFormat(calledArgs[i]) : "";
            var expectedArg = i < args.length ? sinonFormat(args[i]) : "";
            var diff = jsDiff.diffJson(calledArg, expectedArg);
