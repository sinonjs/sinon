...
const { concat, filter, join, map, reduce, slice } = commons.prototypes.array;
...
function throwYieldError(proxy, text, args) {
    let msg = commons.functionName(proxy) + text;
    if (args.length) {
...
const callProto = {
    calledOn: function calledOn(thisValue) {
        if (samsam.createMatcher.isMatcher(thisValue)) {
    ...
    calledWith: function calledWith() {
        ...
        return reduce(
            calledWithArgs,
            function (prev, arg, i) {
                return prev && samsam.deepEqual(self.args[i], arg);
            },
            true,
        );
    },
    ...
    calledWithMatch: function calledWithMatch() {
        ...
        return reduce(
            ...
            function (prev, expectation, i) {
                ...
                return prev && samsam.createMatcher(expectation).test(actual);
            },
            true,
        );
    },
    ...
    returned: function returned(value) {
        return samsam.deepEqual(this.returnValue, value);
    },
    ...
    yieldToOn: function (prop, thisValue) {
        ...
        if (!yieldFn) {
            throwYieldError(
                this.proxy,
                ` cannot yield to '${commons.valueToString(
                    prop,
