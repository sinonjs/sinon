...
- `config.toFake` - _String[ ]_ - an array with explicit function names to fake. By default FakeTimers will automatically fake all methods _except_ `process.nextTick`. You could, however, still fake `nextTick` by providing it explicitly
- `config.shouldAdvanceTime` - _Boolean_ - tells FakeTimers to increment mocked time automatically based on the real system time shift (default: false)
